...
export default function CategoryManagement({ onDeleteRequest }: Props) {
  ...
  return (
    <div className="bg-white dark:bg-neutral-900 rounded-2xl border border-gray-100 dark:border-neutral-800 shadow-sm overflow-hidden transition-colors">
      <div className="px-[10px] py-6 md:p-8 border-b border-gray-100 dark:border-neutral-800 bg-gray-50/30 dark:bg-neutral-800/30">
      ...
      <div className="px-[10px] py-6 border-b border-gray-100 dark:border-neutral-800 flex gap-3">
        <input
          type="text"
      ...
      {categoryMsg && (
        <p className={`px-[10px] py-2 sm:px-6 text-xs font-bold ${categoryMsg.includes('added') ? 'text-green-600' : 'text-red-500'}`}>{categoryMsg}</p>
      )}
      <div className="divide-y divide-gray-50 dark:divide-neutral-800 max-h-72 overflow-y-auto">
        {categories.length === 0 && !loading ? (
          <p className="px-[10px] py-10 sm:px-8 text-center text-gray-400 text-sm font-bold uppercase tracking-widest italic">No categories yet. Add your first!</p>
        ) : (
          categories.map((cat) => (
            <div key={cat.id} className="flex items-center justify-between px-[10px] py-2 sm:px-8 hover:bg-gray-50/30 dark:hover:bg-neutral-800/30 transition-colors group">
